...
select job, max(sal)"급여최고액", min(sal)"급여최저액", sum(sal)"급여총액", round(avg(sal))"평균급여"
from emp
group by job;
...
--26. 관리자 수를 출력하시오.
select count(distinct (mgr)) "관리자 명수"
from emp;
...
--28. 직급별 사원의 최저 급여를 출력하시오.       goup by job
--    관리자를 알 수 없는 사원과                where mgr is not null
--    최저 급여가 2000 미만인 그룹은 제외시키고  (groupby의 having으로)
--    결과를 급여에 대한 내림차순으로 정렬하여 출력하시오.
select job, min(sal)"최저급여"
from emp
where mgr is not null
group by job
having min(sal)>=2000   --having not min(sal)<2000
...
select deptno, count(*)"사원수", round(avg(sal),2)"평균급여"
from emp
group by deptno;

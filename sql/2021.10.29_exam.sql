...
select substr(hiredate, 1,5) "입사연월"
from emp;
...
select max(sal)"급여최고액", min(sal)"급여최저액", sum(sal)"급여총액", round(avg(sal))"평균급여"
from emp;

----24. 각 담당 업무 유형별로 
--      급여 최고액, 최저액, 총액 및 평균 액을 출력하시오. 
--      평균에 대해서는 정수로 반올림 하시오.
select job, max(sal) as "급여최고액", min(sal) as "급여최저액", sum(sal) as "급여총액", round(avg(sal)) as "평균급여"
from emp
group by job;
...
--26. 관리자 수를 출력하시오.
select count(job) "관리자 명수"
from emp
where job = 'MANAGER';

--27. 급여 최고액, 급여 최저액의 차액을 출력하시오.
select max(sal) - min( sal)"최고,최저 차액"
from emp;
...
select deptno, count(deptno)"사원수", round(avg(sal),2)"평균급여"
from emp
group by deptno;
...
select job, deptno, sum(decode(deptno, 10, sal, null)) "부서10 급여총액",
                    sum(decode(deptno, 20, sal, null)) "부서20 급여총액",
                    sum(decode(deptno, 30, sal, null)) "부서30 급여총액",
                    sum(sal) "총액"
from emp
group by job, deptno
;

...
public abstract class Ex03Abstract {
   public static void main(String [] args) {
       //추상클래스는 구현되지 않은 메소드가 존재하므로
       //해당 클래스의 생성자를 우리가 직접 호출할 수 없다
       //대신 그 클래스를 상속받는 클래스의 생성자가 필요로하다
       
       AnimalA a = new Cat();
//       a.makeSound();
//       a.move();
       observe(a); 
       //추상메소드인 메잌사운드와 무브가 존재한다(애니멀을 상속받았으니)/네시오십이분 다시들어
       
       // 인터페이스도 추상클래스와 매우 유사하지만
       // 달라지는 점은, 실제로 구현되는 메소드가 존재하는 추상클래스와 다르게

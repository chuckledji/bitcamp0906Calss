...
//      b.id = nextId++;
    ...
    public static void printList() {
        if (list.isEmpty()) {
            ...
            while (!validateId(userChoice)) {
                System.out.println("해당글번호는 존재하지않는다");
    ...
    public static void printOne(int id) {
        int index = indexOf(id);//이부분뭔지알아보기
        // 파라미터로들어온 id를 찾아서 넣으면 된다
    ...
    public static void update(int id) {
        ...
        // 인덱스변수를 만들어서 list에 인덱스값을 넣어 토대로 찾게된다
    ...
    // 파라미터로 들어온 id값과 일치하는 객체의 인덱스를 찾는
    // indexOf()
    public static int indexOf(int id) {
        BoardTver b = new BoardTver();
        b.setId(id);
        // 이러면 이퀄즈에서트루가 나오는애들 찾을수있다고?

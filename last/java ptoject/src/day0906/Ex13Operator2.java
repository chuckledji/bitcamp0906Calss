...
public class Ex13Operator2 {
    ...
    // 하지만 변수의 앞에 붙나 뒤에 붙나에 따라서 의미가 달라진다. 
    // ++, --가 변수의 앞에 붚을 때에는 전위증가(감소)연산자 라고 부르고
    ...
    // 2. string 값인 "++number: " 뒤에 이어져서
    // "++number: 2"라는 스트링 값이 만들어진다.(스트링으로 변환된 것)
    // 3. System.out.println이 () 안의 값인 "++number: 2" 를 화면에 출력시킨다
    ...
    // 후의 증감연산자는 해당 줄에서 가장 마지막에 실행되는 코드가 된다.
    System.out.println("number의 현재 값: " + number);
    System.out.println("number++: " + number++);
    // 위 코드는 전위증감연산자와 마찬가지로 한줄로 적혀있지만
    // 실제론 크게 3단계로 나뉘어서 코드가 실행이 된다.
    // 1. String 값 "number++: " 뒤에 int 변수 number의 현재 값인
    //    2가 이어져서 "number++: 2"라는 String값이 만들어진다. 
    // 2. System.out.println()이 () 안의 "number++: 2" 라는 겂을 화면에 출력시킨다.

package day0913;
//배열(Array)
// 배열이란 우리가 지정한 갯수만큼의 똑같은 케이터타입의 변수들을
// 한개의 이름으로 묶어서 처리를 하는 자료형이다.

// 예를 들어서 우리가 점수 4개를 score1, score2, score3, score4 이렇게
// 따로따로 변수르 만드는 것이 아니라 길이가 4인 int 배열을 만들어서
// 0번째 칸, 1번째 칸, 2번째 칸, 3번째 칸에 각각 값을 저장하거나 호출하는 방식을 사용하겠다 라는 의미가 된다.
...
public class Ex02Array {
 public static void main(String[] args) {
     ...
     //배열을 초기화할때는 해당 배열이
     // 총 몇개의 변수를 관리할 것인지를 반드시 지정해주어야한다.
     // 좀 더 쉬운 표현으로는 총 몇칸짜리 배열인지를 지정해주어야한다.
     
     //intArray를 3칸짜리 배열로 초기화해보자
     intArray = new int[3];
     
     //우리가 일반적인 int변수는 
     //각각의 int변수의 이름을 호출하여 관리하였지만
     ...
     // 배열에 있는 특정 인덱스번 칸에 있는 값을 호출할때에는
     //배열이름[인덱스]로 호출할 수 있따.
    ...
    //1. 초기화시에 길이는 우리가 다른 변수를 사용하요 처ㅗ기화할 수 있다
    int size = 4;
    intArray = new int[size];
    
    //2.해당 배열의 길이는
    // 배열이름.length로 알아볼 수 있따.
    System.out.println("intArray.length: " + intArray.length);

    //3. 우리가 특정 인덱스의 값을 출력하거나 값을 지정할 때에는
    // 인덱스의 값을 직접 써줘도 되지만 int변수를 적어주어도 된다.
   ...
   // 주소값은 부여가 되어있지만 해당 주소값을 참조하여 heap영역으로 가면
   // 아직 해당 공간이 확보가 안되어있는 상태이다.
   // null일 때에는 아무것도 실핼시킬 수 없다.
 }
}

package day0907;
// printf()
// printf()는 출력할 내용을 %문자를 사용하여 형식에 맞춰 출력할 수 있게 만들어준다.
...
public class Ex04PrintF {
    ...
    //데이터 손실을 막기 위해 명령을 무시한다.
    // D. 10진법 정수를 왼쪽정렬 5자리로 출력해라
    ...
    // A. 16진법 정수를 그대로 출력하되, 알파벳이 존재할경우 소문자로 출력해라.
    System.out.printf("2-A. [%x]\n", number);
    
    // B. 16진법 정수를 그대로 출력하되 알파벳이 존재할시 대문자로 출력해라
    ...
    // D. 16진법 정수를 5자리 왼쪽정렬로 출력하되 알파벳이 존재할 경우 소문자로 출력해라
    System.out.printf("2-D. [%-5x]\n", number);
    
    // E. 16진법 정수를 5자리 오른쪽 정렬로 맞추고 왼쪽 빈자리는 0으로 채우고 알파벳은 대문자로 출력해라
    ...
    // C. 스트링을 8자리 오른쪽 정렬하여 그대로 출력해라
    System.out.printf("4-C. [%8s]\n", str);
        
    // D. 스트링을 8자리 왼쪽 정렬하여 모두 대문자로 출력해라
    System.out.printf("4-D. [%-8S]\n", str);
    
    // 문자이므로 빈공백에 0을 넣는 것은 없습니다.
    ...
    // System.out.printf("%f\n", 3);
    // 3. printf와 관련된 예제는 여러가지 %문자를 조합해서
    // 우리가 원하는대로 형식을 맞추어서 출력하게 되는데
    // 이때, %문자의 갯수가 뒤의 값의 갯수보다 적으면 문제없다.
    //System.out.printf("%d %f %d %s", 1, 1.1, 2, "33", 4, 5, 6);
    // 하지만 반대로 %분자의 갯수가 뒤의 값의 갯수보다 많으면 에러가 발생한다.

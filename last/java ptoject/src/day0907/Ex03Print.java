...
// 2. println():
//    print a line의 줄임말으로서, ()안의 내용을 그대로 출력하고,
//    그 다음 출력할 내용이 있으면, 출력된 내용의 다음줄의 첫번째 칸부터 다시 출력을 시작한다.
// 3. printf():
//    print in format의 줄임말로서, ()안의 내용을 형식에 맞추어 출력하고
//    그 다음 출력할 내용이 있으면, 출력된 내용의 오른쪽 칸부터 다시 출력을 시작한다.

// 이스케이프 문자
// 이스케이프 문자란, 공백이나 문장 부호들의 특수한 기호에서 사용되는 문자로서
// \와 조합돼서 사용이 된다.
// 이스케이프 문자는 주로 2글자로 이루어져있지만, 실제 문자로서는 한 글자로 취급이 된다.
...
public class Ex03Print {
    public static void main(String[] args) {
        ...
        System.out.println(); // ()빈줄은 str2의 오른쪽칸부터 출력이 시작될 것이다.
        System.out.println("-------------------------");
        System.out.println();
        ...
        // 이스케이프 문자
        // 이스케이프 문자는 특수한 공백(탭 겅백이나 빈줄 공백) 혹은
        ...
        // 4. \" ("를 나타내고 싶으면 이렇게 적어줘야한다)
        //    코드 상에서 "는 String의 시작 혹은 끝을 뜻하게 된다.
        System.out.println("4. \"");
        // """이라고 하면 스트링 값이 시작 종료 후 다시 새로운 스트링 값이 곧장 시작되고 끝나지 않고, 두개의 값이 연산자 없이
        // 곧장 붙어있기 때문에 에러가 발생한다.
        // 따라서, 문자"를 적어주기 위해서는 이스케이프 문자인\" 써주어야 한다.

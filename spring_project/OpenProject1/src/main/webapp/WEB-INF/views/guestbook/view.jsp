	...
	<script>
		$(document).ready(function() {
			...
			$('#replyWriteForm').submit(function() {
				...
				${}
				$.ajax({
					/*http://localhost:8080/op/guestbook/eply/write*/
					
					url : '${pageContext.request.contextPath}/api/v1/guestbook/reply/'+ idx,	//'reply/delete'	
					type : 'DELETE'																//'POST',
					data : $(this).serialize(),
		...
		function deleteReply(idx){
			...
			if(confirm('댓글을 삭제하시겠습니까?')){
				...
				$.ajax({
					url : 'reply/delete',			/*http://localhost:8080/op/guestbook/eply/delete*/
					type : 'post',
					//data : {idx : idx},				//전달받은 idx get과 delete는 body에 추가가 어렵다.pathvariable을 이용하도록
					success : function(data){
						if(data == '1'){
							// #reply4
							$('#reply'+idx).remove();
